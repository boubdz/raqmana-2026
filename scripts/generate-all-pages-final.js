...
// إزالة أجزاء TypeScript بطريقة ذكية
// 1. إزالة الأسطر التي تبدأ بـ import, export interface, interface, type
...
for (let line of lines) {
  ...
  // تخطي الأسطر التي تبدأ بكلمات مفتاحية معينة
  ...
  // تخطي تعريفات الواجهات المتعددة الأسطر (تقريبي)
  if (trimmed.includes('ServiceCategory') && trimmed.includes('{')) insideInterface = true;
  if (insideInterface && trimmed.includes('}')) {
...
let jsContent = cleanedLines.join('\n');

// 2. إزالة علامات الاستفهام والأنواع (مثل phone?: string → phone:)
jsContent = jsContent.replace(/\?:\s*[^,\n]+/g, ':');
// إزالة الكلمات الدالة على الأنواع
jsContent = jsContent.replace(/:\s*(string|number|boolean|any|void|null|undefined|\[\]|Record<[^>]+>|React\.\w+|[A-Z]\w*\[\]?)/g, ':');
// إزالة الفواصل الزائدة بعد إزالة الأنواع
jsContent = jsContent.replace(/:\s*,/g, ',');
// إزالة export const serviceCategories -> const serviceCategories
jsContent = jsContent.replace('export const serviceCategories', 'const serviceCategories');
// إزالة الكلمة readonly
jsContent = jsContent.replace(/\breadonly\b/g, '');

// 3. تقييم النص مباشرة (بدون حفظ ملف مؤقت)
let serviceCategories;
try {
  // إضافة الأقواس لتحويل العبارة إلى كائن صالح
  const evalStr = `(function() { ${jsContent}; return serviceCategories; })()`;
  serviceCategories = eval(evalStr);
  console.log(`✅ تم تحليل ${serviceCategories.length} فئة بنجاح.`);
} catch (err) {
  console.error('❌ فشل التقييم:', err.message);
  process.exit(1);
}

// دالة توليد صفحة HTML (مختصرة وجاهزة)
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .service-card { background: #f9f9f9; border-radius: 8px; padding: 12px; border: 1px solid #e0e0e0; transition: all 0.2s; }
        .service-card:hover { background: #eef2ff; transform: translateY(-2px); }

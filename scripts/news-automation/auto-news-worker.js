...
1. [العنوان الرئيسي H1]: ⚠️ شرط تقني صارم: يجب أن يكون العنوان بين 50 و 65 حرفاً بالضبط (محركات البحث تقطع أي عنوان أطول من 65 حرفاً في نتائجها). مثال:
   # 📋 دليل [اسم الخدمة المختصر] 2026: الشروط والتسجيل
   ❌ ممنوع: عناوين أكثر من 65 حرفاً مهما كانت الأسباب.
...
async function processSite(site, history, articlesCount) {
  ...
  // تحميل قائمة العناوين الموجودة لتفادي التكرار الموضوعي
  let existingTitles = [];
  try {
    const existingData = JSON.parse(fs.readFileSync(CONFIG.ARTICLES_JSON, 'utf8'));
    existingTitles = Object.values(existingData)
      .filter(a => a.autoGenerated)
  ...
  try {
    ...
    for (const item of items) {
      ...
      // 1b. فحص التكرار الموضوعي — منع مقالات عن نفس الموضوع تقريباً
      const titleWords = (item.title || '').toLowerCase().split(/\s+/).filter(w => w.length > 3);
      const isDuplicateTopic = existingTitles.some(existing => {
        const matchCount = titleWords.filter(w => existing.includes(w)).length;
        return matchCount >= 4; // 4 كلمات مشتركة تعني نفس الموضوع
      });
      if (isDuplicateTopic) {
        console.log(`   ⏩ موضوع مشابه موجود مسبقاً: ${item.title?.slice(0, 50)}`);
        history.processedItems[item.guid || item.link] = { skippedReason: 'duplicate_topic', skippedAt: new Date().toISOString() };
        continue;
      }
      ...
      try {
        ...
        try {
          await sendOneSignalPush({
            ...
            message: `${article.introduction ? article.introduction.slice(0, 90) : 'دليل تفصيلي جديد في المنصة'} (اضغط للمتابعة)`,
            url: articleUrl,
          });
        } catch {}
        */

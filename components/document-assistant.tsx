...
export function DocumentAssistant() {
  ...
  // Check saved unlock on mount & parse query params
  useEffect(() => {
    try {
    ...
    try {
      if (typeof window !== "undefined" && window.location.search) {
        ...
        if (urlDocType === "cv" || urlDocType === "resume") {
          ...
          setDocType("request");
          setTone("formal");
          setDescription(`طلب إداري رسمي بخصوص: ${urlDocType}.\n\nأرجو صياغة طلب خطي رسمي موجه للإدارة المختصة يتضمن المعلومات الشخصية والوضعية الإدارية والمبررات القانونية الثبوتية بالصيغة الرسمية المعتمدة.`);
        }
      }
    } catch {}
  }, []);
  ...
  return (
    <div className="space-y-12 max-w-6xl mx-auto pb-20" dir="rtl">
      ...
      {/* ─── Viral Facebook Share Gate Banner ─────────────────────── */}
      {!isEmailUnlocked ? (
        <div className="rounded-[2.5rem] bg-gradient-to-br from-indigo-950/95 via-blue-950/95 to-slate-950/95 border border-primary/30 p-8 md:p-12 text-white shadow-2xl backdrop-blur-xl relative overflow-hidden">
          <div className="absolute -right-10 -bottom-10 h-48 w-48 rounded-full bg-primary/20 blur-3xl pointer-events-none" />
          <div className="absolute -left-10 -top-10 h-32 w-32 rounded-full bg-blue-500/20 blur-2xl pointer-events-none" />
          
          <div className="max-w-2xl mx-auto text-center space-y-6 relative z-10">
            <div className="inline-flex h-16 w-16 items-center justify-center rounded-2xl bg-[#1877F2]/20 border border-[#1877F2]/40 shadow-inner">
              <Share2 className="h-8 w-8 text-[#1877F2]" />
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-white/80 text-sm md:text-base leading-relaxed">
                صياغة غير محدودة للطلبات الإدارية، العرائض القانونية ونماذج الطعون بالذكاء الاصطناعي مجاناً. شارك الرابط على فيسبوك لتفعيل المساعد الذكي فوراً.
              </p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-right bg-white/5 border border-white/10 p-4 rounded-2xl">
              <p className="text-white/90 text-xs font-medium">✅ صياغة قانونية سليمة بالعربية الفصحى</p>
              <p className="text-white/90 text-xs font-medium">✅ تحميل مباشر وطباعة فورية للوثائق</p>
              <p className="text-white/90 text-xs font-medium">✅ الدال على الخير كفاعله.. ساعد زملائك</p>
              <p className="text-white/90 text-xs font-medium">✅ لا احتكار للمعلومة والخدمات الرقمية</p>
            </div>
        ...
        <div className="flex items-center justify-between px-6 py-3.5 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-xs font-bold text-emerald-600 dark:text-emerald-400">
          <div className="flex items-center gap-2">
            <Zap className="h-4 w-4" />
            <span>المساعد الذكي مُفعل بالكامل — شكراً لمشاركتك الخدمة مع زملائك ✨</span>
          </div>
          <button
            onClick={() => {
              localStorage.removeItem("raqmana_shared_facebook");
